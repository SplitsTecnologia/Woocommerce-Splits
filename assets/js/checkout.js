/* global wcsplitsParams, splitsCheckout */
(function( $ ) {
	'use strict';
	...
	$( function() {
		...
		$( 'form.checkout' ).on( 'checkout_place_order_splits-credit-card', {
			preservesplitsCheckoutSubmitValue: true
		}, isCheckoutInvalid );
		...
		function isCheckoutInvalid( evt ) {
			...
			if ( formSubmit ) {
				if ( 'undefined' !== typeof evt && 'undefined' !== typeof evt.data ) {
					if ( 'undefined' !== typeof evt.data.preservesplitsCheckoutSubmitValue && ! evt.data.preservesplitsCheckoutSubmitValue ) {
						formSubmit = false;
					}
				}
				return true;
			...
			if ( 0 < $( 'input[name=splits_checkout_token]' ).length ) {
				return true;
			}
		...
		function openCheckout() {
			...
			var checkout, customer, params,
				...
				inline_data = $( '#splits-checkout-params', form );

			// Create checkout.
			checkout = new SplitsCheckout.Checkout({
				encryption_key: wcsplitsParams.encryptionKey,
				success: function( data ) {
					formSubmit = true;
					// Remove any old token input.
					$( 'input[name=splits_checkout_token]', form ).remove();

					// Add the token input.
					form.append( $( '<input name="splits_checkout_token" type="hidden" />' ).val( data.token ) );
			...
			if ( wcsplitsParams.checkoutPayPage ) {
				customer = wcsplitsParams.customerFields;
			} else {
			...
			params = $.extend({}, {
				...
				createToken:      true,
				interestRate:     wcsplitsParams.interestRate,
				maxInstallments:  inline_data.data( 'max_installment' ),
				freeInstallments: wcsplitsParams.freeInstallments,
				postbackUrl:      wcsplitsParams.postbackUrl,
				uiColor:          wcsplitsParams.uiColor
			}, customer );
