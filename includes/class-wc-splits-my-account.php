<?php
/**
 * Splits Tecnologia My Account actions
 *
 * @package WooCommerce_splits/Frontend
 */
...
/**
 * WC_splits_My_Account class.
 */
class WC_splits_My_Account {
	...
	public function my_orders_banking_ticket_link( $actions, $order ) {
		if ( 'splits-banking-ticket' !== $order->payment_method ) {
			return $actions;
		}
		...
		if ( ! empty( $data['boleto_url'] ) ) {
			$actions[] = array(
				'url'  => $data['boleto_url'],
				'name' => __( 'Print Banking Ticket', 'woocommerce-splits' ),
			);
		}
